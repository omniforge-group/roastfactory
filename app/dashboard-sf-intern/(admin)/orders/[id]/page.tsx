...
export default function OrderDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/dashboard-sf-intern/orders/${id}`)
      ...
      .then(data => { setOrder(data); setStatus(data.status); setLyrics(data.lyrics || ""); setLoading(false); })
  ...
  async function saveChanges() {
    ...
    setSaving(false);
    setSaveMsg(res.ok ? "✅ Opgeslagen" : "❌ Opslaan mislukt");
    if (res.ok) setOrder(prev => prev ? { ...prev, status, lyrics } : prev);
...
- Lengte: ${lengthMap[o.package] ?? "1 couplet + refrein"}`;
  ...
  function generateSunoPrompt(o: Order): string {
    ...
    const bpmMap: Record<string, string> = { mild: "120bpm", medium: "130bpm", savage: "140bpm", nuclear: "150bpm" };
    const moodMap: Record<string, string> = {
      mild: "fun and playful", medium: "comedic and sharp", savage: "brutal and funny", nuclear: "no mercy",
    };
    const battleExtra = o.package === "battle_mode" ? ", battle rap format, two verses alternating" : "";
    return `Style: ${genreMap[o.roast_level] ?? "upbeat pop"}, Dutch lyrics, roast song about ${o.roast_target}, ${o.occasion} theme, ${o.roast_level} intensity, catchy hook, ${bpmMap[o.roast_level] ?? "120bpm"}${battleExtra}

Mood: ${moodMap[o.roast_level] ?? "fun and playful"}`;
  }
  ...
  if (loading) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 80, color: "#555" }}>
      Laden...
    </div>
  );

  if (!order) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 80, color: "#FF2D2D" }}>
      Order niet gevonden.
    </div>
  );
  ...
  return (
    <div style={{ color: "#fff", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <style>{`textarea:focus { border-color: #FF2D2D !important; outline: none !important; } select:focus { border-color: #FF2D2D !important; outline: none !important; }`}</style>

      {/* Breadcrumb */}
      <div style={{ padding: "14px 32px", borderBottom: "1px solid #1a1a1a", display: "flex", alignItems: "center", gap: 16 }}>
        <Link href="/dashboard-sf-intern/bestellingen" style={{ color: "#555", fontSize: 13, textDecoration: "none", fontWeight: 600 }}>← Bestellingen</Link>
        <span style={{ color: "#222" }}>|</span>
        <span style={{ fontSize: 14, fontWeight: 700 }}>{order.customer_name || order.id}</span>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px", display: "grid", gridTemplateColumns: "1fr 340px", gap: 24, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ background: "#111", border: "1px solid #222", borderRadius: 14, padding: 24 }}>
            ...
            <div style={{ background: "#0A0A0A", border: "1px solid #FF2D2D22", borderRadius: 10, padding: 16 }}>
            ...
            {order.extra_info && (
              <>
                ...
                <div style={{ background: "#0A0A0A", border: "1px solid #222", borderRadius: 10, padding: 16 }}>
          ...
          {order.audio_url && (
            <div style={{ background: "#111", border: "1px solid #222", borderRadius: 14, padding: 24 }}>
              ...
              <a href={order.audio_url} target="_blank" style={{ fontSize: 13, color: "#FF6B00", textDecoration: "none" }}>↗ Download link</a>
            </div>
          )}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20, position: "sticky", top: 24 }}>
          ...
          <div style={{ background: "#1a1a1a", border: "1px solid #FF2D2D55", borderRadius: 14, padding: 24 }}>
            ...
            <p style={{ margin: "0 0 14px", fontSize: 12, color: "#555" }}>Gebruik deze prompts om de roast te maken</p>
            <div style={{ display: "flex", gap: 4, marginBottom: 14 }}>
              {(["lyrics", "suno"] as const).map(tab => (
                <button key={tab} onClick={() => setActivePromptTab(tab)} style={{
                  ...
                  {tab === "lyrics" ? "Lyrics Prompt" : "Suno Stijl"}
                </button>
              ))}
            </div>
            <textarea
              ...
              style={{
                width: "100%", height: 200, background: "#0A0A0A", border: "1px solid #2a2a2a",
                borderRadius: 8, padding: "12px 14px", color: "#ccc", fontSize: 12,
                fontFamily: "monospace", lineHeight: 1.7, resize: "none",
                boxSizing: "border-box", marginBottom: 10, overflow: "auto",
              }}
            />
            <button onClick={() => copyPrompt(activePromptTab)} style={{
              width: "100%", padding: 11, borderRadius: 8, border: "none",
              background: copiedTab === activePromptTab ? "#22C55E" : "#FF2D2D",
              color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", transition: "background 0.15s",
            }}>
          ...
          <div style={{ background: "#111", border: "1px solid #222", borderRadius: 14, padding: 24 }}>
            ...
            <textarea value={lyrics} onChange={e => setLyrics(e.target.value)}
              placeholder="Voer hier de roast tekst in..."
              style={{ width: "100%", minHeight: 200, background: "#0A0A0A", border: "1px solid #333", borderRadius: 8, padding: "12px 14px", color: "#fff", fontSize: 13, resize: "vertical", fontFamily: "inherit", boxSizing: "border-box", marginBottom: 14, lineHeight: 1.7 }}
            />
            <button onClick={saveChanges} disabled={saving}
              style={{ width: "100%", padding: 12, borderRadius: 8, border: "none", background: saving ? "#1a1a1a" : "linear-gradient(135deg, #FF2D2D, #FF6B00)", color: "#fff", fontWeight: 700, fontSize: 14, cursor: saving ? "not-allowed" : "pointer", marginBottom: 8 }}>
          ...
          <div style={{ background: "#111", border: "1px solid #222", borderRadius: 14, padding: 24 }}>
            ...
            <p style={{ margin: "0 0 14px", fontSize: 13, color: "#555", lineHeight: 1.6 }}>
              Upload de MP3 roast naar Supabase Storage.
            </p>
            <input ref={fileRef} type="file" accept=".mp3,audio/mpeg"
              style={{ width: "100%", color: "#888", fontSize: 13, marginBottom: 12 }} />
            <button onClick={uploadAudio} disabled={uploading}
              style={{ width: "100%", padding: 12, borderRadius: 8, border: "1px solid #FF2D2D", background: uploading ? "#1a1a1a" : "#FF2D2D1a", color: uploading ? "#555" : "#FF2D2D", fontWeight: 700, fontSize: 14, cursor: uploading ? "not-allowed" : "pointer", marginBottom: 8 }}>
          ...
          <div style={{ background: "#1a0505", border: "1px solid #FF2D2D55", borderRadius: 14, padding: 24 }}>
            ...
            <p style={{ margin: "0 0 6px", fontSize: 13, color: "#aaa", lineHeight: 1.6 }}>Stuur de roast naar:</p>
            <p style={{ margin: "0 0 16px", fontSize: 14, fontWeight: 700, color: "#FF6B00" }}>{order.customer_email}</p>
            <button onClick={deliverRoast} disabled={delivering}
              style={{ width: "100%", padding: 15, borderRadius: 10, border: "none", background: delivering ? "#1a1a1a" : "linear-gradient(135deg, #FF2D2D, #FF6B00)", color: "#fff", fontWeight: 900, fontSize: 15, cursor: delivering ? "not-allowed" : "pointer", marginBottom: 8 }}>
              {delivering ? "Versturen..." : "🔥 Verstuur roast naar klant"}
            </button>
            {order.delivered_at && (
              <p style={{ margin: 0, fontSize: 12, color: "#22C55E" }}>✅ Verstuurd op {new Date(order.delivered_at).toLocaleString("nl-NL")}</p>
            )}
